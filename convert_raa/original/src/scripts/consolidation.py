...
if __name__ == '__main__':
    ...
    ref = 'raa_nw'
    source_database = ['raa_friesland', 'raa_republiek']
    merge_tables = [
        ...
        ('regionaal', ['bovenlokaalcollegeregentdetails']),
    ...
    for row in c.fetchall():
        alias, idregent = row['alias'], row['IDPersoon']
        c.execute("UPDATE %s.aliassen SET id=%s WHERE alias = \"%s\" AND IDPersoon = %s" % (
